...
function parseDimensionsToLBH(text) {
  ...
  // 1) Format vereinheitlichen
  let scale = 1; // default mm
  let s = raw
            .replace(/[,;]/g, '.')   // Komma/Semikolon → Punkt
            .replace(/\s+/g, '')    // Leerzeichen entfernen
            .replace(/[×xX*/]/g, 'x'); // ×, x, X, * → x

  // 2) Einheiten erkennen & entfernen
  if (/cm\b/i.test(s)) { scale = 10;  s = s.replace(/cm\b/ig, ''); }
  if (/(^|\D)m\b/i.test(s)) { scale = 1000; s = s.replace(/m\b/ig, ''); }
